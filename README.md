...
**Windows 发布包**：[下载 AI Workbench v2.0.1](https://github.com/hhzz-svg/ai-workbench/releases/tag/v2.0.1)
...
1. 从 [AI Workbench v2.0.1 Release](https://github.com/hhzz-svg/ai-workbench/releases/tag/v2.0.1) 下载 `ai-workbench-windows-2.0.1.zip`
2. 解压后双击 `install.bat`
3. 双击 `start.bat`
4. 浏览器打开后进入「设置」，保存 API Base URL、模型和 API Key
5. 回到「新建」，上传资料并开始生成

如果没有保存 API 配置，且电脑里也没有 Codex CLI，创建任务时会提示先去「设置」保存 API。若页面提示无法连接本地后端，请保持 `start.bat` 窗口打开，点击「重新连接」或重新运行 `start.bat`。
