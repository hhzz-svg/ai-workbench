...
本地 Web 工作台，统一集成多种创作类 Skill 接口，把论文、资料和需求交给对应的 Skill，生成演示文稿、学术润色稿、网页演示、学术海报和视觉海报。数据完全本地化，隐私安全可控。
...
- ⚙️ **可接入不同模型配置**：支持本机默认 Codex 环境，也可以保存自定义 API Base URL、模型和 Key。
...
# 第一次使用:双击运行 install.bat,然后双击 start.bat
# 后续使用:直接双击 start.bat
```
